...
### 4.1. PiCo-M
The logic of the [Persons in Context model](https://personsincontext.org) is easily applicable to data on enslaved persons. However, PiCo-M needs to be excluded.
...
| 2. | Age categories | hasAge is used for exact ages <br> _**We suggest not to use hasAge to refer to age categories**_ | **no** |
| 3. | Enumerations | Each individual in an enumeration can be modelled as an enslaved with the PiCo or extended characteristics, provided that such enumerations contain descriptive information. For example, 5 underage, male enslaved belonging to J. Janssen | **yes** |
...
### 4.2. Enslaved.org
[Enslaved.org](https://docs.enslaved.org/) contains multiple useful properties to model properties and entities unique to enslavement. However, not all required properties and entities are modelled by Enslaved.org and relations cannot be reused as they Enslaved.org models them as separate concepts rather than a direct relationship between two person observations.
